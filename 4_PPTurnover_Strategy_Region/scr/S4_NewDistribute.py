...
#%%
def Newprod2pp(new_prod,old_et,Turnover_dir,CCSInstall_dir,isec,ifa,ireg,yr_beg,yr_end,yearls2):
    ...
    if new_build_style == 'orderly':
        ...
        rec_coun.loc[:,yearls2] = rec_coun.loc[:,yearls2].mask(rec_coun.loc[:,yearls2]==0,-1)
        ...
        mer_prod = pd.merge(record_build.loc[:,['Fake_Plant ID','Start Year']],mer_prod,on='Fake_Plant ID',how='right')

    return mer_prod,dis_reference
...
def NewProdDistr_main(ieng_sc,iend_sc,ior_sc,
    ...
    new_prod_pp,new_op = Newprod2pp(new_prod=new_prod_tot,old_et=old_prod_pp,Turnover_dir=Turnover_dir,CCSInstall_dir=CCSInstall_dir,isec=isec,ifa=ifa,ireg=ireg,yr_beg=yr_beg,yr_end=yr_end,yearls2=yearls2)
